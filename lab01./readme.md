...
В ходе выполнения лабораторной работы я разработал Shell-скрипт `backup.sh`, который автоматизирует процесс резервного копирования важных данных. Создание резервной копии вручную — задача рутинная и подверженная ошибкам, поэтому автоматизация этого процесса значительно упрощает работу и снижает риск потери информации. 
